...
with tables2:
    ...
    for coin in current_invested_symbols:
        ...
        st.markdown(f"<b>Coin: {coin}</b><br>Last Transaction: {last_transaction_date}<br>Purchased Price: {_pp}<br>Current Price: {current_price}<br>Quantity: {quantity}", unsafe_allow_html=True)
